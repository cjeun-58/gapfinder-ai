...
# --- 1. 페이지 설정 및 데이터 초기화 ---
st.set_page_config(page_title="GapFinder AI v3.6", layout="wide")
...
# [STEP 2]
            ...
            if keywords:
                ...
                for kw in kw_list:
                    with st.spinner(f"'{kw}' 수집 중..."):
                        try:
                            with DDGS() as ddgs:
                                res = list(ddgs.text(f"{kw} 블로그 후기", region='kr-kr', max_results=5))
                                all_results.extend(res)
                            time.sleep(1.5)
                        except: st.warning(f"'{kw}' 수집 실패")
            
            if manual_input: all_results.append({'title': '직접 입력', 'body': manual_input})
            
            if not all_results: st.error("수집된 데이터가 없습니다.")
            else:
    ...
    if st.session_state['consumer_summary']: st.info(st.session_state['consumer_summary'])
...
# [STEP 3]
    ...
    if not st.session_state['brand_text'] or not st.session_state['consumer_data']:
        st.error("STEP 1, 2에서 데이터를 먼저 수집해주세요!")
    else:
            ...
            if validate_api_key():
                with st.spinner("전략 도출 중..."):
                    try:
                        ...
                        prompt = "광고 전략가로서 아래 데이터를 분석해 Gap 리포트를 써줘.\n\n"
